...
internal data class State(
	...
	val selectedAuroraReport: AuroraReport? = null
)
...
internal sealed class Change {
	...
	data class AuroraReportSuccess(val place: Place, val report: AuroraReport?) : Change()
		...
		reduce { change ->
			when (change) {
				...
				is Change.PlaceSelected ->
					copy(selectedPlace = change.place, selectedAuroraReport = null) + // TODO Get cached report
					Action.Stream(isStreaming, change.place)
				is Change.StreamToggle -> copy(isStreaming = change.stream) +
